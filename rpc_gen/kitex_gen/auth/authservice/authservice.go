...
func healthCheckHandler(ctx context.Context, handler interface{}, arg, result interface{}) error {
	...
	case *streaming.Args:
		st := s.Stream
		req := new(auth.HealthCheckReq)
		if err := st.RecvMsg(req); err != nil {
			return err
		}
		resp, err := handler.(auth.AuthService).HealthCheck(ctx, req)
		if err != nil {
			return err
		}
		return st.SendMsg(resp)
	case *HealthCheckArgs:
		success, err := handler.(auth.AuthService).HealthCheck(ctx, s.Req)
		if err != nil {
			return err
		}
...
var HealthCheckArgs_Req_DEFAULT *auth.HealthCheckReq

func (p *HealthCheckArgs) GetReq() *auth.HealthCheckReq {
	if !p.IsSetReq() {
...
type HealthCheckResult struct {
	Success *auth.HealthCheckResp
}

var HealthCheckResult_Success_DEFAULT *auth.HealthCheckResp

func (p *HealthCheckResult) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetSuccess() {
		p.Success = new(auth.HealthCheckResp)
	}
	return p.Success.FastRead(buf, _type, number)
}
...
func (p *HealthCheckResult) GetSuccess() *auth.HealthCheckResp {
	if !p.IsSetSuccess() {
...
func (p *HealthCheckResult) SetSuccess(x interface{}) {
	p.Success = x.(*auth.HealthCheckResp)
}
...
func (p *kClient) HealthCheck(ctx context.Context, Req *auth.HealthCheckReq) (r *auth.HealthCheckResp, err error) {
